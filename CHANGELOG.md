...
- `import` now skips directories matching `$_PEEK_EXCLUDE_DIRS`.
...
- Bash/Fish/POSIX/Zsh: `_PEEK_RESOLVE_SYMLINKS` now works on Windows.
...
- Support for Tcsh.
- Added `--score` flag to `peek add`.
- POSIX: add doctor to diagnose common issues.
- Nushell: add CLI completions.

### Changed

- Bash: peek will now automatically `cd` when selecting Space-Tab completions.
...
- PowerShell: avoid setting `$error` when defining `__peek_hooked`.
...
- Zsh: make `__peek_z_complete` available with `--no-cmd`.
...
- Enforce that `$_PEEK_DATA_DIR` is an absolute path.
...
- Symlinks not getting added to the database when `$_PEEK_RESOLVE_SYMLINKS=0`.
...
- Short option `-a` for `peek query --all`.
...
- Fish/Zsh: aliases on `__peek_z` will now use completions.
...
- `peek query --interactive` should not conflict with `--score`.
...
- `peek add` and `peek remove` now accept multiple arguments.
...
- `peek query --all` for listing deleted directories.
...
- Nushell: avoid calling `__peek_hook` on non-filesystem subshells.
- Fish: `alias cd=z` now works, but it must be done after calling `peek init`.
- PowerShell: avoid calling `__peek_hook` on non-filesystem providers.
- Fish: avoid calling `__peek_hook` in private mode.
...
- `peek remove -i` now accepts multiple selections.
- `peek add` no longer accepts zero parameters.
- `$_PEEK_EXCLUDE_DIRS` now defaults to `"$HOME"`.
- Binary releases now use `.zip` on Windows, `.tar.gz` otherwise.

### Fixed

- `cd -` on Fish shells.
- `__peek_hook` no longer changes value of `$?` within `$PROMPT_COMMAND` on
  Bash.
...
- peek unable to find itself on Xonsh shells.
...
- `$_PEEK_EXCLUDE_DIRS` now supports globs.
- `peek init` now defines `__peek_z*` functions that can be aliased as
  needed.
- Support for the [Xonsh](https://xon.sh/) shell.
- `peek import` can now import from Autojump.

### Changed

- `peek init --no-aliases` no longer generates `z` or `zi`.
...
- `$_PEEK_FZF_OPTS` to specify custom options for `fzf`
- `peek query --list` to list all matches
- `peek query --score` to show score along with result

### Changed

- Increased default value of `$_PEEK_MAXAGE` to `10000`.
- Symlinks are treated as separate directories by default, this can be changed
  by setting `_PEEK_RESOLVE_SYMLINKS=1`.

### Removed

- Help menus for `z` and `zri`.
- `peek remove -i` is replaced with `zri`.
...
- Interactive mode for removing entries (`peek remove -i`).
- Aliases for interactive `query` and `remove` (`zqi` and `zri` respectively).
- PWD hooks for POSIX shells.

### Changed

- `peek remove` now throws an error if there was no match in the database.
- Interactive mode in `peek` no longer errors out if `fzf` exits gracefully.
- Canonicalize to regular paths instead of UNC paths on Windows.
- `peek init` now uses PWD hooks by default for better performance.
- `$_PEEK_ECHO` now only works when set to `1`.
...
- `--z-cmd` flag for `peek init` to rename the `z` command to something else.

### Changed

- `peek query` output no longer has the `query:` prefix.
...
- `$_PEEK_EXCLUDE_DIRS` to prevent directories from being added to the database.
...
- Thread safety using unique tempfile names for each `peek` instance.
...
- `$_PEEK_ECHO` to echo match before `cd`ing.
...
- `_peek_precmd` overriding other precmd hooks on Zsh.
...
- Multiple hooks being added upon initializing `peek` multiple times.
...
[0.10.0]: https://github.com/better-ls/peek/compare/v0.9.9...v0.10.0
[0.9.9]: https://github.com/better-ls/peek/compare/v0.9.8...v0.9.9
[0.9.8]: https://github.com/better-ls/peek/compare/v0.9.7...v0.9.8
[0.9.7]: https://github.com/better-ls/peek/compare/v0.9.6...v0.9.7
[0.9.6]: https://github.com/better-ls/peek/compare/v0.9.5...v0.9.6
[0.9.5]: https://github.com/better-ls/peek/compare/v0.9.4...v0.9.5
[0.9.4]: https://github.com/better-ls/peek/compare/v0.9.3...v0.9.4
[0.9.3]: https://github.com/better-ls/peek/compare/v0.9.2...v0.9.3
[0.9.2]: https://github.com/better-ls/peek/compare/v0.9.1...v0.9.2
[0.9.1]: https://github.com/better-ls/peek/compare/v0.9.0...v0.9.1
[0.9.0]: https://github.com/better-ls/peek/compare/v0.8.3...v0.9.0
[0.8.3]: https://github.com/better-ls/peek/compare/v0.8.2...v0.8.3
[0.8.2]: https://github.com/better-ls/peek/compare/v0.8.1...v0.8.2
[0.8.1]: https://github.com/better-ls/peek/compare/v0.8.0...v0.8.1
[0.8.0]: https://github.com/better-ls/peek/compare/v0.7.9...v0.8.0
[0.7.9]: https://github.com/better-ls/peek/compare/v0.7.8...v0.7.9
[0.7.8]: https://github.com/better-ls/peek/compare/v0.7.7...v0.7.8
[0.7.7]: https://github.com/better-ls/peek/compare/v0.7.6...v0.7.7
[0.7.6]: https://github.com/better-ls/peek/compare/v0.7.5...v0.7.6
[0.7.5]: https://github.com/better-ls/peek/compare/v0.7.4...v0.7.5
[0.7.4]: https://github.com/better-ls/peek/compare/v0.7.3...v0.7.4
[0.7.3]: https://github.com/better-ls/peek/compare/v0.7.2...v0.7.3
[0.7.2]: https://github.com/better-ls/peek/compare/v0.7.1...v0.7.2
[0.7.1]: https://github.com/better-ls/peek/compare/v0.7.0...v0.7.1
[0.7.0]: https://github.com/better-ls/peek/compare/v0.6.0...v0.7.0
[0.6.0]: https://github.com/better-ls/peek/compare/v0.5.0...v0.6.0
[0.5.0]: https://github.com/better-ls/peek/compare/v0.4.3...v0.5.0
[0.4.3]: https://github.com/better-ls/peek/compare/v0.4.2...v0.4.3
[0.4.2]: https://github.com/better-ls/peek/compare/v0.4.1...v0.4.2
[0.4.1]: https://github.com/better-ls/peek/compare/v0.4.0...v0.4.1
[0.4.0]: https://github.com/better-ls/peek/compare/v0.3.1...v0.4.0
[0.3.1]: https://github.com/better-ls/peek/compare/v0.3.0...v0.3.1
[0.3.0]: https://github.com/better-ls/peek/compare/v0.2.2...v0.3.0
[0.2.2]: https://github.com/better-ls/peek/compare/v0.2.1...v0.2.2
[0.2.1]: https://github.com/better-ls/peek/compare/v0.2.0...v0.2.1
[0.2.0]: https://github.com/better-ls/peek/compare/v0.1.1...v0.2.0
[0.1.1]: https://github.com/better-ls/peek/compare/v0.1.0...v0.1.1
[0.1.0]: https://github.com/better-ls/peek/commits/v0.1.0
